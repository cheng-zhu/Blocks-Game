...
class Model {
    ...
    Model(Model model) {
        ...
        _hand = model._hand;
        model._history.add(new GameState());
    }
        ...
        if (piece == null) {
            ...
            for (int r = 0; r < piece.height(); r++) {
                for (int c = 0; c < piece.width(); c++) {
                    if (row + r > _height - 1 || col + c > _width - 1) {
                        return false;
                    } else if (piece.get(r, c) && _cells[row + r][col + c]) {
                        return false;
                    }
                }
            }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        return (nrows * _width + ncols * _height)
                * (1 + _streakLength) - Math.min(nrows, ncols);
    }
    ...
    boolean roundOver() {
        for (Piece p : _hand) {
            if (p != null && placeable(p)) {
                return false;
            }
        }
        if (_hand.stream().allMatch(p -> p == null)) {
            return false;
        }
        return true;
    }
